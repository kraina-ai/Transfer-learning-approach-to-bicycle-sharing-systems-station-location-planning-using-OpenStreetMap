...
CLASSIFIERS = [RandomForestClassifier, BalancedRandomForestClassifier, BalancedSubsampleRandomForestClassifier]
    ...
    try:
        ...
        for imb_r in inbalance_ratios:
            for x_processing in x_processings:
                ...
                if tmp_vectors.shape[0] != len(tmp_y) or tmp_vectors.shape[0] != len(tmp_hex_id_list) or len(tmp_y) != len(tmp_hex_id_list):
                    print(imb_r, tmp_vectors.shape, len(tmp_y), len(tmp_hex_id_list))
                # print()

                results_dict[key] = (tmp_vectors, tmp_y, tmp_hex_id_list)
    except Exception as ex:
        print('generate_xy', ex)
            ...
            for iteration in range(10):
                ...
                for (imb_r, x_processing), (x, y, hex_ids) in tuples_dict.items():
                    ...
                    try:
                        ...
                        print('Line 316', ex)
                        print(x.shape, len(y_hex_zip), len(y))
                        raise
                    ...
                    for _ in range(10):
                        try:
                            ...
                            print('Line 333', ex)
                            print(X_tmp.shape, len(Y_hex_id_tmp), len(y_tmp))
                            raise
